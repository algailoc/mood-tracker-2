...
class BadStuffBlock extends StatelessWidget {
  const BadStuffBlock({super.key});
      ...
      return Column(
        ...
        children: [
          const Padding(
            ...
            child: DaySegmentTitle('badStuff'),
          ),
          ...List.generate(state.dayEntity!.badStuff.length + 1, (index) {
            if (index == state.dayEntity!.badStuff.length) {
              return const _NewInput();
            }

            return Padding(
              padding: const EdgeInsets.symmetric(horizontal: 16),
              child: _BadStuffItem(state.dayEntity!.badStuff[index]),
            );
          }),
        ],
      );
...
class _BadStuffItem extends StatelessWidget {
  final String text;

  const _BadStuffItem(this.text);
...
class _NewInputState extends State<_NewInput> {
  ...
  void _onAddPressed() {
    ...
    BlocProvider.of<DayBloc>(context).add(AddBadStuffEvent(controller.text));
